...
## Key-Value Store: Strings, Streams, Lists, & Sorted Sets
...
- Create, add members to, view ranks and scores within, and remove members from sorted sets

This key-value store implementation relies on a Golang mapping from string (key value) to `KeyValue` struct, which is capable of storing a `string`, `stream`, `list`, or `zset` value type. Store operations rely on read & write locks on mutexes to minimize conflicts.
...
See `store/sorted_sets.go`, `store/skip_list.go`, & [Redis Sorted Sets](https://redis.io/docs/latest/develop/data-types/sorted-sets/) for more information on the sorted sets implementation.
